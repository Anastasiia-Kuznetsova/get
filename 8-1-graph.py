...
ax.plot(time, data, lw = 1, c = "b", marker = "o", ms = 5, markevery = 300, label = "V(t)")
#настройка легенды
...
ax.grid(which='major', linestyle='-', linewidth=0.1, color = "silver")
ax.grid(which='minor', linestyle = ':', color = "lightgrey", linewidth = 0.1)

#настройка текста
index = data.argmax()
ax.text(6 * 10, 0.5, 'Время зарядки ' + str(time[index])[:6] + "c", fontsize = 10)
ax.text(6 * 10, 0.7, 'Время разрядки ' + str(time[-1]-time[index])[:6] + "c", fontsize = 10)
